...
const Form = ({
  ...
  const [teamInvitation, setExample] = useState(teamInvitationProps);
  ...
  const generateInputFields = () => {
    ...
    const newData = {};
    fields.map((f) => {
      newData[f.key] = actionType === 'new' ? f.default : teamInvitation.input_data[f.key];
    })
  ...
  const handleSubmit = (e) => {
    ...
    if (actionType === 'new') {
      createRequest(
        `/teams/${team.id}/teamInvitations.json`,
      ...
      updateRequest(
        `/teams/${team.id}/team_invitations/${teamInvitation.id}.json`,
        {  },
        redirectOnSuccess,
