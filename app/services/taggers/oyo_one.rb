module Taggers
  class OyoOne
    ...
    def tags_for(object)
      tags = []
      data = object.input_data.symbolize_keys
      tags << property_tag(data[:property_type])
      tags << location_detail_tags(data[:location_detail])
      tags << target_user_tags(data[:target_user])
      tags << usp_tags(data[:usp_one])
      tags << usp_tags(data[:usp_two])
      tags << usp_tags(data[:usp_three])
